...
function socketCallback(data) {
  showMessage(data);
  console.log(METAVERSE.getScene("Client").size + "/" + METAVERSE.getScene().size);
}

function sendMessage() {
  const message = document.getElementById("message").value;
  showMessage("> " + message);
  METAVERSE.send(message);
}
...
function inspectObject(event) {
  ...
  var obj = METAVERSE.scene.get(objectId);
  if (inspecting == null) {
    inspecting = objectId;
...
function createField(event) {
  ...
  var obj = METAVERSE.scene.get(inspecting);
  METAVERSE.createField(inspecting.split(' ')[0], fieldName, function(val) {
...
function inspectDelete() {
  const obj = METAVERSE.scene.get(inspecting);
...
function inspectSubmit() {
  var obj = METAVERSE.getScene().get(inspecting);
	var changed = false;
	...
	if ( changed ) {
		...
		METAVERSE.send(JSON.stringify(event));
	}
  createSceneButton(inspecting);
  inspecting = null;
  inputFields = [];
...
window.addEventListener('load', function() {
  import("/babylon/metaverse.js").then((module)=>{
    METAVERSE=module.METAVERSE;
    METAVERSE.addConnectionListener(setConnected);
    METAVERSE.addDataListener(socketCallback);
    METAVERSE.addSceneListener(sceneChanged);
