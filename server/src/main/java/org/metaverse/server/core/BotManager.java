...
import org.metaverse.server.config.BotConfig;
import org.metaverse.server.config.BotConfig.BotProperties;
import org.metaverse.server.obj.Bot;
...
public class BotManager implements ApplicationListener<ContextRefreshedEvent> {
  ...
  private Class<? extends Bot> getBotClass(String className) throws Exception {
    if (className.indexOf('.') < 0) {
      className = "org.metaverse.server.obj." + className;
    }
