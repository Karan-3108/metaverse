import { World, OpenViduStreams } from '../../../babylon/js/metaverse-min.js';
...
export class Classroom extends World {
  ...
  entered( welcome ) {
    ...
    this.scene.onPointerPick = (e,p) => {
      ...
      if ( p.pickedMesh.name === this.screenShareMesh.name) {
        if ( ! this.screenShare ) {
          ...
          this.worldManager.METAVERSE.createSharedObject({
    ...
    this.worldManager.METAVERSE.addSceneListener( (sceneEvent) => {
  ...
  deleteSharedObject() {
    if ( this.screenShare ) {
      this.worldManager.METAVERSE.deleteSharedObject(this.screenShare);
    }
  }
