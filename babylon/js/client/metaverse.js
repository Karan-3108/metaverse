...
export class VRObject {
  constructor() {
    ...
    this.listeners = [];
    /** Handy reference to Metaverse instance */
    this.METAVERSE = null;
  ...
  */
  ...
  publish() {
    ...
    for ( var key in this ) {
      if ( key !== 'id' && key !== 'METAVERSE' ) {
        event.changes[key] = this[key];
      }
    }
    // FIXME: TypeError: cyclic object value
    let json = JSON.stringify(event);
    this.METAVERSE.log(json);
    this.METAVERSE.send(json);
  }
}
...
export class Metaverse {
  ...
  */
  ...
  @param url optional websocket url, defaults to /metaverse on the same server
   */
  connect(url) {
    if ( ! url ) {
      ...
      //var end = url.lastIndexOf('/'); // localhost:8080/babylon/metaverse
      let end = url.indexOf('/', start+2); // localhost:8080/metaverse      
      url = webSocketProtocol+':'+url.substring(start,end)+'/metaverse'; // ws://localhost:8080/metaverse
    }
...
export const METAVERSE = new Metaverse();