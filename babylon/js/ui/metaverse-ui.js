...
export class MetaverseUI {
  ...
  constructor( ) {
    ...
    this.contentBase = '';
    /** Path to logo, null defaults to contentBase+/babylon (metaverse.org logo)*/
    this.logoPath = null;
    /** Logo file name, defaults to logo.glb */
    this.logoFile = "logo.glb";
    /** metaverse.org logo mesh */
    this.logo = null;
    ...
    /** reference to Metaverse singleton */
    this.METAVERSE = METAVERSE;
    /** reference to AssetLoader singleton */
    this.assetLoader;
  }

  /** Preloads metaverse.org logo and portal for later use 
  @param scene
  */
  async init(scene) {
    if ( ! this.initialized || this.scene !== scene ) {
      ...
      this.logo.name = "Metaverse.org Logo";
  ...
  /** Used in init, return logPath if exists, or default path to metaverse.org logo */
  logoDir() {
    if ( this.logoPath ) {
  ...
  */
  ...
  @param dir directory to list
  @param callback to call, recieves ServerFolder list as argument
...
//export const METAVERSEUI = new MetaverseUI();

export let METAVERSEUI;

if (window.METAVERSEUI === undefined) {
  METAVERSEUI = new MetaverseUI();
  window.METAVERSEUI=METAVERSEUI;
} else {
  METAVERSEUI = window.METAVERSEUI;
}
