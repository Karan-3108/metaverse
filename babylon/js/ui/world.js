...
export class World {
  ...
  constructor(params) {
    ...
    /** Handy reference to MetaverseUI */
    this.METAVERSEUI = METAVERSEUI;
  ...
  */
  async init(engine, name, scene, callback, baseUrl, file) {
    ...
    if ( ! this.onProgress ) {
      this.indicator = METAVERSEUI.loadProgressIndicator(this.scene, this.camera);
  ...
  */
  ...
  async load(callback) {
    ...
    var promises = [];
    if ( this.file ) {
      var scenePromise = METAVERSEUI.assetLoader.loadAsset( this.baseUrl+this.file,
        // onSuccess:
        (url, container, info ) => {
    ...
    if ( this.worldObjects ) {
      ...
      for ( var url in this.worldObjects ) {
        ...
        instances.forEach( (instance) => {
          var objPromise = METAVERSEUI.assetLoader.loadAsset(url,
            // callback 
  ...
  async loadAsset(relativePath, file) {
    return METAVERSEUI.assetLoader.loadAsset(this.assetPath(relativePath)+file);
  }
