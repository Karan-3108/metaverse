...
export class WorldEditor {
  constructor( world, fileInput ) {
    ...
    this.contentBase=METAVERSEUI.contentBase;
    ...
    world.isSelectableMesh = (mesh) => {
      return this.worldPickPredicate(mesh) || METAVERSEUI.findRootNode(mesh).VRObject;
    }
  }
  ...
  makeUI() {
    ...
    this.guiManager = METAVERSEUI.guiManager;
  ...
  makeAButton(text, imageUrl, action) {
    var button = METAVERSEUI.hud.addButton(text,imageUrl);
  ...
  objectLoaded( vrObject, rootMesh ) {
    ...
    if ( vrObject.properties && vrObject.properties.editing == this.worldManager.METAVERSE.me.id ) {
      METAVERSEUI.indicator.remove("Download");
      ...
      if ( ! vrObject.scale ) {
        ...
        setTimeout( () => {
          ...
          this.worldManager.METAVERSE.sendEvent(vrObject, {scale: { x:scale, y:scale, z:scale }} );
        }, 100 );
      } else {
  ...
  resizeObject(obj) {
    ...
    var resizeHandler = this.scene.onPointerObservable.add((pointerInfo) => {
      ...
      if ( pointerInfo.type == BABYLON.PointerEventTypes.POINTERUP ) {
        ...
        if ( pointerInfo.pickInfo.hit && METAVERSEUI.findRootNode(pointerInfo.pickInfo.pickedMesh) == obj ) {
          ...
          this.worldManager.METAVERSE.sendEvent(obj.VRObject, {scale: { x:scale, y:scale, z:scale }} );
        }
      }
    });
  }
      ...
      if ( pointerInfo.type == BABYLON.PointerEventTypes.POINTERUP ) {
        ...
        if ( pointerInfo.pickInfo.hit && METAVERSEUI.findRootNode(pointerInfo.pickInfo.pickedMesh) == obj ) {
          ...
          if ( Math.abs(dxz) > Math.abs(dy*3) ) {
            ...
            this.worldManager.METAVERSE.sendEvent(obj.VRObject, {rotation: { x:obj.rotation.x, y:result.y, z:obj.rotation.z}} );
          } else {
            // rotating around all axes
            this.worldManager.METAVERSE.sendEvent(obj.VRObject, {rotation: { x:result.x, y:result.y, z:result.z}} );
          }
  ...
  alignObject(obj) {
    ...
    var pickInfo = this.scene.pickWithRay(ray, (mesh) => {
      var pickedRoot = METAVERSEUI.findRootNode(mesh);
      return pickedRoot != obj;
    });
    console.log(pickInfo);
    var y = obj.position.y - pickInfo.distance;
    this.worldManager.METAVERSE.sendEvent(obj.VRObject, {position: { x:obj.position.x, y:y, z:obj.position.z }} );
  }
  
  upright(obj) {
    this.worldManager.METAVERSE.sendEvent(obj.VRObject, {rotation: { x:0, y:obj.rotation.y, z:0 }} );
  }

  removeObject(obj) {
    this.worldManager.METAVERSE.deleteSharedObject(obj.VRObject);
  }
  ...
  displayButtons(show) {
    METAVERSEUI.hud.showButtons(show);
    if ( show ) {
      this.activeButton = null;
    }
  }
  ...
  relocatePanel() {
    ...
    var forwardDirection = METAVERSEUI.hud.camera.getForwardRay(6).direction;
    this.uiRoot.position = METAVERSEUI.hud.camera.position.add(forwardDirection);
    this.uiRoot.rotation = new BABYLON.Vector3(METAVERSEUI.hud.camera.rotation.x,METAVERSEUI.hud.camera.rotation.y,METAVERSEUI.hud.camera.rotation.z);
    //this.panel.linkToTransformNode(this.uiRoot);
    this.displayButtons(true);
  }
  ...
  installClickHandler() {
    this.clickHandler = this.scene.onPointerObservable.add((pointerInfo) => {
      if ( pointerInfo.pickInfo.pickedMesh ) {
        var pickedRoot = METAVERSEUI.findRootNode(pointerInfo.pickInfo.pickedMesh);
  ...
  take(vrObject, position) {
    ...
    // default position
    if ( ! position ) {
      var forwardDirection = METAVERSEUI.hud.camera.getForwardRay(2).direction;
      var forwardLower = forwardDirection.add(new BABYLON.Vector3(0,-.5,0));
      position = METAVERSEUI.hud.camera.position.add(forwardLower);
    ...
    var targetDirection = position.subtract(METAVERSEUI.hud.camera.position);
    var forwardDirection = METAVERSEUI.hud.camera.getForwardRay(targetDirection.length()).direction;
    ...
    if ( vrObject.rotation ) {
      ...
      quat = BABYLON.Quaternion.Inverse(METAVERSEUI.hud.camera.absoluteRotation).multiply(quat);
      target.rotation = quat.toEulerAngles()
    }
    target.parent = METAVERSEUI.hud.camera;
  ...
  sendPos(obj) {
    var rot = METAVERSEUI.hud.camera.rotation;
    var pos = obj.position;
    if ( obj.target ) {
      pos = obj.target.absolutePosition;
      rot = obj.target.absoluteRotationQuaternion.toEulerAngles();
    }
    this.worldManager.METAVERSE.sendEvent(obj, {position: { x:pos.x, y:pos.y, z:pos.z }, rotation: {x:rot.x, y:rot.y, z:rot.z}} );
  }
  ...
  editObject(obj, editing) {
    if ( editing ) {
      obj.properties.editing = this.worldManager.METAVERSE.me.id;
    } else {
      obj.properties.editing = null;
    }
    this.worldManager.METAVERSE.sendEvent(obj, {properties: obj.properties} );
  }
  ...
  save() {
    this.displayButtons(true);
    var dump = METAVERSEUI.assetLoader.dump();
    if ( Object.keys(dump).length > 0 ) {
      METAVERSEUI.saveFile(this.world.name+".json", JSON.stringify(dump));
    }
  }
  ...
  publish( objects ) {
    for ( var url in objects) {
      ...
      instances.forEach( (instance) => {
        ...
        this.worldManager.METAVERSE.createSharedObject(mesh, (obj)=>{
  ...
  download(result) {
    ...
    METAVERSEUI.indicator.animate();
    METAVERSEUI.indicator.add("Download");
  ...
  createSharedObject( mesh, properties ) {
    var object = {
      mesh: mesh,
      properties: {editing: this.worldManager.METAVERSE.me.id},
    ...
    this.worldManager.METAVERSE.createSharedObject(object, (obj)=>{
      console.log("Created new VRObject", obj);
    });
  }
