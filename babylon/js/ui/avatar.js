...
export class Avatar {
  ...
  dispose() {
    if ( this.character ) {
      METAVERSEUI.assetLoader.unloadAsset(this.getUrl(), this.instantiatedEntries);
  ...
  */
  ...
  load(success, failure) {
    this.loadFixes().then( () => {
      ...
      var plugin = METAVERSEUI.assetLoader.loadAsset(
        this.getUrl(),
        // onSuccess:
        (loadedUrl, container, info, instantiatedEntries ) => {
          ...
          if ( instantiatedEntries ) {
            ...
            try {
              ...
              METAVERSEUI.assetLoader.unloadAsset(this.getUrl());
              if ( failure ) {
                failure(exception);
              } else {
  ...
  saveAnimations(groupName) {
    for ( var i = 0; i < this.character.animationGroups.length; i++ ) {
      ...
      if ( animationGroup.name === groupName ) {
        ...
        this.attachAnimations(group);
        METAVERSEUI.saveFile(animationGroup.name+'.json', json);
        return;
      }
    }
