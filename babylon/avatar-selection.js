import { METAVERSEUI, World, Buttons, LogoRoom, Portal, WorldManager, Avatar, VideoAvatar, AvatarController, OpenViduStreams, ServerFile } from './js/metaverse-min.js';
...
export class AvatarSelection extends World {
  constructor() {
    ...
    this.serverUrl = null;
    /** content base, defaults to METAVERSEUI.contentBase */
  ...
  listAnimations() {
    METAVERSEUI.listDirectory( this.animationDir(), animations => {
  ...
  createSelection(selectionCallback) {
    ...
    METAVERSEUI.listMatchingFiles( this.characterDir(), (folders) => {
  ...
  async createAvatarSelection(folder) {
    ...
    if ( folder.url ) {
      METAVERSEUI.listCharacters( folder.url(), (avatars) => {
  ...
  async createCustomAvatar() {
    ...
    this.customAvatarFrame.src = `https://metaverse.readyplayer.me/avatar?frameApi`;
  ...
  showPortals() {
    this.portals = {};
    METAVERSEUI.listThumbnails(this.worldDir(), (worlds) => {
  ...
  async enterWorld( worldUrl, worldName, avatarUrl = this.avatarUrl(), worldScript  = 'world.js') {
    ...
    import(worldUrl+'/'+worldScript).then((world)=>{
      var afterLoad = (world) => {
        ...
        this.worldManager.METAVERSE.debug = this.debug; // network debug
  ...
  dispose() {
    ...
    if ( this.character ) {
      this.character.dispose();
      //METAVERSEUI.assetLoader.unloadAsset(this.character.getUrl());
      this.character = null;
    }
    */
